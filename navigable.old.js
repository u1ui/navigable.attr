...
document.addEventListener('u1-target', e => {
	...
	if (oldTarget && oldTarget.matches('dialog[u1-navigable]')) {
		if (target && !target.contains(oldTarget)) oldTarget.close();
	}
    if (target && target.matches('dialog[u1-navigable]')) {
...
addEventListener('close',e=>{
	...
	if (!target.matches('dialog[u1-navigable]')) return;
...
document.addEventListener('u1-target', e => {
	const {oldTarget,target} = e.detail;
	if (oldTarget && oldTarget.matches('details[u1-navigable]')) {
		if (target && !target.contains(oldTarget)) oldTarget.open = false;
		//oldTarget.open = false;
	}
	if (target && target.matches('details[u1-navigable]')) {
		e.target.open = true;
	}
});
addEventListener('toggle',e=>{
	const target = e.target;
	if (!target.id) return;
	if (!target.matches('details[u1-navigable]')) return;
	if (target.open) {
